<template>
  ...
  ></TopNav>
    ...
    <div class="login-wrap">
      <image style="width: 100%; height: 326px;" src="./images/img_login.png"></image>
      <nut-button plain size="large" type="primary" @click="wxLoginHandle">微信授权登录</nut-button>
      <nut-toast :msg="pracTextMsg" type="text" v-model:visible="show"></nut-toast>
    </div>
</template>
...
const _wechatLogin = async (code) => {
  ...
  // if (res.code === 200 && res.data) {
    // 缓存用户信息
    // 缓存token
    if (res.token) {Taro.setStorageSync('token', res.token)};
    // 登录之后调用, 获取用户当前阶段
    _getCurUserStage()
  }
// }
      ...
      if (data.status !== 1) {
        ...
        if (data.pracType) {
          ...
          if (data.userInfoBool) {
            ...
            Taro.navigateTo({url: '/pages/guides/userInfo'})
          }
        } else {
          ...
          pracTextMsg.value = data.pracText.result
          setTimeout(() => {
            Taro.navigateTo({url: '/pages/action/psychologicalTests/index'})
          }, 500)
        }
      } else {
        // 提示用户，测试结果不正常，终止使用app其他流程
        console.log(data.pracText.msg)
        Taro.navigateTo({url: '/pages/action/psychologicalTests/remind'})
      }
    } else {
      console.log(message)
    }
