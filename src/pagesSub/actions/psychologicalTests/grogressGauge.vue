<template>
  ...
  ></TopNav>
    ...
    <main class="grogress-gauge">
        <div class="gauge">
            <image style="width:252px; height:145px;" src="@/assets/images/action/graph@2x.png"></image>
...
const goNextHandle = () => {
    ...
    if (psychometricIdArr[num + 1]) {
        ...
        // 继续答题
        Taro.redirectTo({url: '/pagesSub/actions/psychologicalTests/topic'})
    } else {
        goNextText.value = '下一步'
        Taro.setStorageSync('curSubPsychometricId', null)
        Taro.setStorageSync('psychometricIdArr', null)
        // 跳转到显示雷达图结果页
        Taro.redirectTo({url: '/pagesSub/actions/psychologicalTests/done'})
    }
}
...
onMounted (() => {
    let option = {
        series: [
            ...
            pointer: {
                icon: 'image://../../../../assets/images/icons/icon_home_1@2x.png',
                length: '75%',
                width: 16,
                offsetCenter: [0, '5%']
            },
